...
class CellpackSession extends microb_1.Cellpack {
    ...
    setSessionCookie(sid, connection, session) {
        connection.response.setCookie(new microb_1.Cookie(this.config.name, sid, session.getExpires() || this.config.expires, session.getPath() || "/", session.getDomain() || "", session.isSecure() || false, session.isHttponly() || true));
    }
    generateSid() {
        return new Promise((resolve, reject) => {
            Uid(24).then((uid) => {
